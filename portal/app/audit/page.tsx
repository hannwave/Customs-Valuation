...
function AuditValuationEvidence({ record }: { record: AuditRecord }) {
  ...
  const originalPrice = values.originalPrice === null || values.originalPrice === undefined || values.originalPrice === "" ? "Not recorded" : String(values.originalPrice);
  const currency = typeof values.originalPriceCurrency === "string" ? values.originalPriceCurrency : "";
  return <div className="audit-evidence-attachment"><strong>Product evidence</strong><div className="audit-evidence-images">{photo ? <img src={photo} alt="Uploaded product" /> : <span>No product photo uploaded</span>}{receipt ? <img src={receipt} alt="Uploaded receipt" /> : <span>No receipt photo uploaded</span>}</div><span><small>Original item price</small><b>{originalPrice}{currency ? ` ${currency}` : ""}</b></span></div>;
}
