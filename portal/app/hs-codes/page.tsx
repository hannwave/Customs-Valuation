...
type FormState = { revisionId: string; code: string; descriptionEn: string; descriptionAm: string; tariffItemNo: string; tariffDescription: string; unit: string; duty: string; sourceReference: string; effectiveDate: string };
const emptyForm: FormState = { revisionId: "", code: "", descriptionEn: "", descriptionAm: "", tariffItemNo: "", tariffDescription: "", unit: "", duty: "", sourceReference: "", effectiveDate: new Date().toISOString().slice(0, 10) };
...
export default function HsCodesPage() {
  ...
  async function loadTree(search = query) {
    ...
    try {
      const [tree, rows] = await Promise.all([request(`/api/hs-catalogue/tree?summary=true${search.trim() ? `&search=${encodeURIComponent(search.trim())}` : ""}`), request("/api/hs-revisions")]);
      setSections(tree.sections ?? []); setRevisions(rows ?? []);
  ...
  return <main className="hs-catalogue-page">
    <header className="hs-catalogue-hero"><div><span className="hs-catalogue-eyebrow">Tariff reference</span><h1>HS catalogue</h1><p>Navigate the tariff book from Section to Chapter, Tariff Item, and the complete HS code record.</p></div><button type="button" className="hs-catalogue-primary" onClick={openCreate}><FiPlus /> Create HS code</button></header>
