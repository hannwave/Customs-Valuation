...
export default function AboutPage() {
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      gsap.from(".about-hero-copy > *", { y: 16, autoAlpha: 0, duration: .62, stagger: .1, ease: "power2.out" });
      [".about-mission article", ".about-why-grid > *", ".about-values-grid article", ".impact-inner > *", ".about-team > *", ".team-card-grid article", ".about-access > div > *"].forEach(selector => {
        gsap.from(selector, { y: 16, autoAlpha: 0, duration: .52, stagger: .08, ease: "power2.out", scrollTrigger: { trigger: selector, start: "top 88%", once: true } });
      });
    }, page);
    return () => context.revert();
  }, []);
  return <main className="about-page" ref={page}>
    <header className="landing-nav"><Link href="/" className="landing-brand" aria-label="Ethiopia Customs Commission home"><Brand variant="dark" /></Link><nav aria-label="Main navigation"><Link href="/">Home</Link><Link className="is-active" href="/about">About</Link><Link href="/contact">Contact</Link></nav><div className="landing-nav-actions"><ThemeToggle /><button className="landing-language" type="button"><FiGlobe /> EN <FiChevronDown /></button><Link href="/login" className="nav-sign-in"><FiUser /> Sign in</Link></div></header>
    ...
    <section className="about-team landing-wrap"><article><p className="about-kicker">Our team <span /></p><h2>People, process and technology.</h2><p>Our team is made up of dedicated professionals — customs officers, IT experts, analysts and support staff — working together to build a modern, efficient and customer-focused customs administration.</p><a className="about-outline" href="#contact">Meet our team <FiArrowRight /></a></article><div className="team-card-grid">{teamItems.map(([TeamIcon, title, text]) => <article key={title}><div className="team-card-header"><i><TeamIcon /></i><h3>{title}</h3></div><p>{text}</p></article>)}</div></section>
