...
export default function AdministrationPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      } else if (current.user.role === "CustomsAdministrator") {
        const response = await authApi<{ requests: RegistrationRequest[] }>("/officer-registration-requests");
        pending = response.requests;
      }
  ...
  async function approve(id: string) {
    ...
    const endpoint = profile?.user.role === "CustomsAdministrator" ? "officer-registration-requests" : "registration-requests";
    const message = profile?.user.role === "CustomsAdministrator" ? "The Customs Officer application was approved and the account is ready for sign-in." : "The registration was approved with the employee's requested branch assignment.";
    try { await authApi(`/${endpoint}/${id}/approve`, { method: "POST" }); setNotice(message); await load(); }
  ...
  async function deny(id: string) {
    ...
    if (profile?.user.role === "CustomsAdministrator" && reason.length < 10) { setError("Provide a denial reason of at least 10 characters."); setBusy(""); return; }
    const endpoint = profile?.user.role === "CustomsAdministrator" ? "officer-registration-requests" : "registration-requests";
    try { await authApi(`/${endpoint}/${id}/deny`, { method: "POST", body: JSON.stringify({ reason }) }); setNotice(profile?.user.role === "CustomsAdministrator" ? "The Customs Officer application was denied and recorded." : "The registration request was denied and recorded."); await load(); }
  ...
  return <div className="management-page">
    ...
    {profile.user.role === "SystemAdministrator" && <section className="admin-panel">
      <div className="panel-heading"><div><h2>Pending Customs Administrator requests</h2><p>Applicants submit their own credentials and requested branch. You can review the request, but passwords and usernames remain private.</p></div><button type="button" onClick={() => void load()}><FiRefreshCw /> Refresh</button></div>
       {requests.length === 0 ? <DataState kind="empty" compact title="No pending administrator applications" description="New Customs Administrator applications will appear here." /> : <div className="table-wrap"><table><thead><tr><th>Applicant</th><th>Department</th><th>Requested branch</th><th>Submitted</th><th>Review</th></tr></thead><tbody>{requests.map(request => { const requestedLocation = request.locationId ? profile.locations.find(location => location.id === request.locationId) : null; return <tr key={request.id}><td><strong>{request.fullName}</strong><small>{request.email}<br />{request.staffId}</small></td><td>{request.department || "-"}</td><td>{requestedLocation ? locationLabel(requestedLocation) : request.locationId ? request.locationId.slice(0, 8) : "Not selected"}</td><td>{new Date(request.submittedAt).toLocaleDateString()}</td><td><div className="request-review-actions"><button className="approve-button" type="button" disabled={busy === request.id} onClick={() => void approve(request.id)}>{busy === request.id ? "Approving..." : "Approve"}</button><input aria-label={`Denial reason for ${request.fullName}`} placeholder="Reason to deny (10+ characters)" value={reviewReasons[request.id] ?? ""} onChange={event => setReviewReasons(values => ({ ...values, [request.id]: event.target.value }))} /><button className="secondary-button danger-button" type="button" disabled={busy === `deny-${request.id}`} onClick={() => void deny(request.id)}>{busy === `deny-${request.id}` ? "Denying..." : "Deny"}</button></div></td></tr>; })}</tbody></table></div>}
    </section>}
    ...
    <section className="admin-panel">
      ...
      {employees.length === 0 ? <DataState kind="empty" compact title={profile.user.role === "SystemAdministrator" ? "No Customs Administrator accounts" : "No manageable employees"} description={profile.user.role === "SystemAdministrator" ? "Approved Customs Administrator accounts will appear here." : "Customs Officers appear here after an approved location assignment."} /> : visibleEmployees.length === 0 ? <DataState kind="empty" compact title="No matching employees" description="Try a different name, email, staff ID, or responsibility." /> : <div className="table-wrap"><table className="management-table"><thead><tr><th>Employee</th><th>Role</th><th>Status</th><th>Primary location</th><th>Responsibilities / reason</th><th>Action</th></tr></thead><tbody>{visibleEmployees.map(row => {
        ...
        return <tr key={row.user.id}><td><strong>{row.user.fullName}</strong><small>@{row.user.username}<br />{row.user.email}<br />{row.user.employeeNumber || "No staff ID"}</small></td><td>{roleLabel(row.user.role)}</td><td><select aria-label={`Status for ${row.user.fullName}`} value={draft?.status ?? "ACTIVE"} onChange={event => setDrafts(values => ({ ...values, [row.user.id]: { ...values[row.user.id], status: event.target.value } }))}><option>ACTIVE</option><option>PENDING_VALIDATION</option><option>SUSPENDED</option><option>INACTIVE</option><option>LOCKED</option></select>{row.user.archivedAt && <small>Archived {new Date(row.user.archivedAt).toLocaleDateString()}</small>}</td><td><select aria-label={`Location for ${row.user.fullName}`} value={draft?.locationId ?? ""} onChange={event => setDrafts(values => ({ ...values, [row.user.id]: { ...values[row.user.id], locationId: event.target.value } }))}><option value="">Select</option>{activeLocations.map(location => <option key={location.id} value={location.id}>{locationLabel(location)}</option>)}</select>{row.user.role === "CustomsAdministrator" && <label className="inline-check"><input type="checkbox" checked={draft?.includeChildren ?? false} onChange={event => setDrafts(values => ({ ...values, [row.user.id]: { ...values[row.user.id], includeChildren: event.target.checked } }))} />Include children</label>}<small>{row.assignments?.length ?? (row.locationId ? 1 : 0)} active assignment{(row.assignments?.length ?? (row.locationId ? 1 : 0)) === 1 ? "" : "s"}</small></td><td><input aria-label={`Responsibilities for ${row.user.fullName}`} placeholder="Valuation, Inspection" value={draft?.responsibilities ?? ""} onChange={event => setDrafts(values => ({ ...values, [row.user.id]: { ...values[row.user.id], responsibilities: event.target.value } }))} /><input aria-label={`Change reason for ${row.user.fullName}`} placeholder="Reason (10+ characters)" value={draft?.reason ?? ""} onChange={event => setDrafts(values => ({ ...values, [row.user.id]: { ...values[row.user.id], reason: event.target.value } }))} /></td><td><div className="employee-row-actions"><button className="approve-button" type="button" disabled={busy === row.user.id || !draft?.locationId || (draft?.reason.length ?? 0) < 10} onClick={() => void updateEmployee(row.user.id)}>{busy === row.user.id ? "Saving…" : "Save"}</button><button className="secondary-button" type="button" onClick={() => void openActivity(row)}><FiActivity />Activity</button><button className="secondary-button danger-button" type="button" disabled={!row.user.active || busy === `archive-${row.user.id}`} onClick={() => { setArchiveEmployee(row); setArchiveReason(""); setError(""); }}><FiArchive />Archive</button></div></td></tr>;
      })}</tbody></table></div>}
    </section>
  </div>;
}
