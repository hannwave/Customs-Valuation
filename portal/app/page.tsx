...
function AdminDashboard({ data, profile }: { data: WorkspaceDashboard; profile: WorkspaceProfile }) {
  ...
  return <>
    <DashboardHeader profile={profile} eyebrow="Regional employee administration" title={root?.displayName || root?.name || "My region"} description="Manage officers and review their activity within your assigned region." actions={<div className="dashboard-quick-actions"><Link href="/administration"><FiUsers />Manage officers</Link><Link href="/audit"><FiActivity />Audit activity</Link></div>} />
    <KpiGrid data={data} />
    <div className="dashboard-main-grid">
      ...
      <section className="dashboard-panel dashboard-panel--full"><div className="dashboard-panel-heading"><div><p className="eyebrow">Employee activity</p><h2>Recent actions in my region</h2><span>Events before an officer joined this region are excluded.</span></div><Link href="/audit">Full audit <FiArrowRight /></Link></div><AuditList data={data} /></section>
    </div>
  </>;
}
