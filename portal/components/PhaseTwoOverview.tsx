...
export function PhaseTwoOverview() {
  ...
  async function loadCase(id = caseId) {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(normalized)) {
      setError("Enter the Phase 1 valuation case ID to continue.");
      return;
    }
    setBusy("load");
    setError("");
    setNotice("");
    try {
      const response = await loadPhase2(normalized);
      setData(response);
      setDraft(requestFromPhase2(response));
      setNotice("Phase 1 case loaded. Phase 2 can now be saved as an extension of this case.");
    } catch (ex) {
      setError(ex instanceof Error ? ex.message : "The Phase 1 case could not be loaded.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function findHsCode(event: FormEvent<HTMLFormElement>) {
    ...
    if (hsSearch.trim().length < 2) {
      setError("Enter at least two characters to search HS codes.");
      return;
    }
    setBusy("search");
    setError("");
    try {
      const response = await searchPhase2HsCodes(hsSearch.trim());
      setHsResults(response.items);
      setNotice(response.items.length ? "Select the HS code to carry into Phase 2." : "No HS codes matched that search.");
    } catch (ex) {
      setError(ex instanceof Error ? ex.message : "HS codes could not be searched.");
    } finally {
      setBusy(null);
    }
  }
  ...
  function updateTax(index: number, patch: Partial<Phase2TaxLineRequest>) {
    setDraft(current => ({
      ...current,
      taxLines: current.taxLines.map((line, lineIndex) => lineIndex === index ? { ...line, ...patch } : line),
    }));
  }
  ...
  function payload(): Phase2Request {
    return {
      ...
      taxLines: draft.taxLines.map((line, index) => ({
        ...line,
  ...
  async function calculate() {
    if (!data) {
      setError("Load a saved Phase 1 case before calculating Phase 2.");
      return;
    }
    setBusy("calculate");
    setError("");
    setNotice("");
    try {
      setData(await calculatePhase2(data.decisionId, payload()));
      setNotice("Provisional Phase 2 calculation updated. Save the draft to retain it.");
    } catch (ex) {
      setError(ex instanceof Error ? ex.message : "The Phase 2 calculation could not be completed.");
    } finally {
      setBusy(null);
    }
  }
  ...
  async function save(complete = false) {
    if (!data) {
      setError("Load a saved Phase 1 case before saving Phase 2.");
      return;
    }
    ...
    setNotice("");
    try {
      const response = await savePhase2(data.decisionId, payload(), complete);
      setData(response);
      setDraft(requestFromPhase2(response));
      setNotice(complete ? "Phase 2 completed and audited." : "Phase 2 draft saved.");
    } catch (ex) {
      setError(ex instanceof Error ? ex.message : "The Phase 2 draft could not be saved.");
    } finally {
      setBusy(null);
    }
  }
  ...
  return <section className="phase-two-workspace">
    ...
    <div className="phase2-grid">
      <div className="phase2-main">
        ...
        <section className="card phase2-relief-card">
          <div className="panel-heading"><div><p className="eyebrow">RELIEF</p><h2>Exemptions & waivers</h2><p>Enter only the reductions that apply to this case.</p></div></div>
          <div className="phase2-relief-fields">
            <label>Exemption amount<input type="number" min="0" step="0.01" value={draft.exemptionAmount} onChange={event => setDraft(current => ({ ...current, exemptionAmount: Number(event.currentTarget.value) }))} /></label>
            <label>Waiver amount<input type="number" min="0" step="0.01" value={draft.waiverAmount} onChange={event => setDraft(current => ({ ...current, waiverAmount: Number(event.currentTarget.value) }))} /></label>
          </div>
          <details className="phase2-optional-adjustment">
            <summary>Manual adjustment (optional)</summary>
            <div className="phase2-relief-fields">
              <label>Adjustment type<select value={draft.manualAdjustmentType} onChange={event => setDraft(current => ({ ...current, manualAdjustmentType: event.currentTarget.value as "Fixed" | "Percentage" }))}><option value="Fixed">Fixed amount</option><option value="Percentage">Percentage</option></select></label>
              <label>Adjustment value<input type="number" min="0" step="0.01" value={draft.manualAdjustmentAmount} onChange={event => setDraft(current => ({ ...current, manualAdjustmentAmount: Number(event.currentTarget.value) }))} /></label>
            </div>
            <label className="wide-label">Justification and notes<textarea rows={3} value={draft.notes} onChange={event => setDraft(current => ({ ...current, notes: event.currentTarget.value }))} placeholder="Add a short supporting note." /></label>
          </details>
        </section>
      </div>
