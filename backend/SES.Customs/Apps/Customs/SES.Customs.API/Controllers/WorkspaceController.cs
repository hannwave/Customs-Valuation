...
public sealed class WorkspaceController(CustomsDbContext db, WorkspaceAccess access) : ControllerBase
{
    ...
    public async Task<IActionResult> Dashboard(CancellationToken ct)
    {
        ...
        return Ok(new {
            ...
            employees = managedUsers.Take(12).Select(u => new { user = PublicUser(u), locationId = u.PrimaryLocationId, assignments = grants.Where(g => g.UserId == u.Id).Select(g => new { g.CustomsLocationId, g.IncludeChildLocations, g.Responsibilities }) }),
            decisions = decisions.Take(12).Select(d => new { d.Id, d.HsCodeId, hsCode = d.HsCodeId.HasValue ? hsCodes.GetValueOrDefault(d.HsCodeId.Value)?.Code ?? "" : "", product = d.HsCodeId.HasValue ? hsCodes.GetValueOrDefault(d.HsCodeId.Value)?.DescriptionEn ?? "Unknown product" : "Product classification deferred to Phase 2", d.SelectedReferenceValue, d.Currency, d.Decision, d.Status, d.RecordedAt, d.LocationId }),
    ...
    public async Task<IActionResult> CreateEmployee(EmployeeCreate input, CancellationToken ct)
    {
        ...
        var user = new AuthAccountEntity { Id = Guid.NewGuid(), Username = input.Username!, FullName = input.FullName.Trim(), Email = input.Email.Trim().ToLowerInvariant(), Role = role!, Active = true, Status = "ACTIVE", PasswordHash = AuthService.Hash(input.Password!), CreatedAt = now, PrimaryLocationId = input.LocationId, RegionKey = region, RegionJoinedAt = now, EmployeeNumber = input.EmployeeNumber?.Trim() ?? "", Phone = input.Phone?.Trim() ?? "" };
        db.AuthAccounts.Add(user);
        db.UserLocationScopes.Add(new UserLocationScope { Id = Guid.NewGuid(), UserId = user.Id, CustomsLocationId = input.LocationId, IncludeChildLocations = input.IncludeChildren, Responsibilities = role == AccessRules.Officer ? "Valuation and assessment officer" : "Customs Administrator", EffectiveFrom = now, CreatedBy = access.UserId.ToString() });
