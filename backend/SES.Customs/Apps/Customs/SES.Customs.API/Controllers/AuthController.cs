...
public sealed class AuthController(AuthService auth, IConfiguration configuration) : ControllerBase
{
    ...
    {
        if (string.IsNullOrWhiteSpace(request.FullName) || string.IsNullOrWhiteSpace(request.StaffId) || string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Department) || string.IsNullOrWhiteSpace(request.Role) || string.IsNullOrWhiteSpace(request.Password)) return BadRequest(new { message = "Complete all required fields." });
    ...
    public IActionResult Me() => Ok(new { id = User.FindFirstValue(JwtRegisteredClaimNames.Sub), username = User.Identity?.Name, email = User.FindFirstValue(ClaimTypes.Email), fullName = User.FindFirstValue("full_name"), role = User.FindFirstValue(ClaimTypes.Role) });
    ...
    private string Token(AuthUser user)
    {
        ...
        var claims = new[] { new Claim(JwtRegisteredClaimNames.Sub, user.Id.ToString()), new Claim(JwtRegisteredClaimNames.UniqueName, user.Username), new Claim(ClaimTypes.Name, user.Username), new Claim(ClaimTypes.Email, user.Email), new Claim("full_name", user.FullName), new Claim(ClaimTypes.Role, user.Role) };
