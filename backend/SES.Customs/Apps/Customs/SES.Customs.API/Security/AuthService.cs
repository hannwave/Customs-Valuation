...
public sealed class AuthService(CustomsDbContext db)
{
    ...
    public async Task<AuthUser> ApproveAsync(Guid id, CancellationToken ct = default)
    {
        ...
        var location = await db.CustomsLocations.FirstOrDefaultAsync(l => l.Status == "ACTIVE" && (l.SupportsValuation || l.SupportsInspection), ct)
            ?? throw new InvalidOperationException("No active valuation office is configured.");
        ...
        var user = new AuthAccountEntity { Id = Guid.NewGuid(), Username = username, Email = request.Email, FullName = request.FullName, EmployeeNumber = request.StaffId, Phone = request.Phone ?? "", Role = SES.Customs.Core.Models.AccessRules.Officer, Active = true, Status = "ACTIVE", PrimaryLocationId = location.Id, PasswordHash = request.PasswordHash, CreatedAt = DateTimeOffset.UtcNow };
        db.AuthAccounts.Add(user);
        db.UserLocationScopes.Add(new UserLocationScope { Id = Guid.NewGuid(), UserId = user.Id, CustomsLocationId = location.Id, Responsibilities = "Valuation and assessment officer", CreatedBy = "approval" });
        request.Status = "Approved"; await db.SaveChangesAsync(ct); return Map(user);
    }
