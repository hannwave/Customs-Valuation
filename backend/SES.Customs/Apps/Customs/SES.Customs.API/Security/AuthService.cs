...
public sealed record AuthUser(Guid Id, string Username, string Email, string FullName, string Role, bool Active, string PasswordHash);
...
public sealed class AuthService(CustomsDbContext db)
{
    ...
    {
        var request = new RegistrationRequestEntity { Id = Guid.NewGuid(), FullName = fullName, StaffId = staffId, Email = email, Phone = phone, Department = department, Role = role, PasswordHash = Hash(password), Status = "Pending", SubmittedAt = DateTimeOffset.UtcNow };
    ...
    {
        var user = new AuthAccountEntity { Id = Guid.NewGuid(), Username = username, Email = email, FullName = fullName, Role = role, Active = true, PasswordHash = passwordIsHash ? password : Hash(password), CreatedAt = DateTimeOffset.UtcNow };
    ...
    public async Task<AuthUser> ApproveAsync(Guid id, CancellationToken ct = default)
    {
        ...
        var user = new AuthAccountEntity { Id = Guid.NewGuid(), Username = username, Email = request.Email, FullName = request.FullName, Role = request.Role, Active = true, PasswordHash = request.PasswordHash, CreatedAt = DateTimeOffset.UtcNow };
    ...
    private static AuthUser Map(AuthAccountEntity u) => new(u.Id, u.Username, u.Email, u.FullName, u.Role, u.Active, u.PasswordHash);
